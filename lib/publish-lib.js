...
async function geminiImage(prompt,env){
 ...
 for(const key of keys){
  ...
  try{const lr=await fetch("https://generativelanguage.googleapis.com/v1beta/models?key="+key);const lj=await r2j(lr);
...
async function r2j(r){return await r.json()}
...
export async function publishTo({platform,message,imageUrl,videoUrl},env){
 ...
 try{
  ...
  if(platform==="instagram"){
   ...
   if(!img)return{ok:false,error:"Gambar tidak tersedia (kuota Nano Banana habis) — lampirkan gambar lalu Retry."};
